...
// (80-series, fj-cruiser, land-cruiser-fj) have one dominant spec → no axes.
export const SPEC_OPTIONS: Record<string, Partial<Record<SpecAxis, SpecOption[]>>> = {
  ...
  // 78-series (n≈7) and 100-series omitted: too sparse / engine is a price wash —
  // they fall back to model+year, which already handles them honestly.
  '200-series': {
    grade: [
  ...
  'prado-150': {
    ...
    engine: [
      { value: 'gd',     label: '2.8 GD diesel', re: /2\.?8|\bgd\b/i },
      { value: '3.0d',   label: '3.0 diesel',    re: /3\.?0|tdi|\bdt\b/i },
      { value: 'petrol', label: '4.0 V6 petrol', re: /4\.?0|\bv6\b/i },
    ],
  },
  'prado-250': {
    grade: [
      { value: 'grs', label: 'GR-Sport',      re: /gr[\s-]?s(port)?/i },
      { value: 'fe',  label: 'First Edition', re: /first[\s-]?edition|\bfe\b/i },
      { value: 'vxl', label: 'VX-L',          re: /vx[\s-]?l/i },
      { value: 'tx',  label: 'TX',            re: /\btx\b/i },
    ],
  },
