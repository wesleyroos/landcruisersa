...
async function ingest() {
  ...
  for (const ref of refs) {
    ...
    if (result.networkError) {
      ...
      if (++consecFail >= ABORT_CONSEC) {
        console.error(`[wbc] ABORTING — ${consecFail} uploads failed in a row; ${result.target} unreachable.`);
        aborted = true;
        break;
      }
      continue;
    ...
    consecFail = 0;
    if (!result.ok) {
      console.error(`[wbc] ingest failed for ${ref.source_id}: ${result.status} @${result.target}`);
      skipped++;
      continue;
    }
  ...
  const removed = await reconcileAllTargets({
    source: 'wbc', refs, scrapedSegments: scrapedSegmentsFor(collectExtra), aborted, capHit: discoverStats.capHit,
  });
